    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 lg:py-14">
      ...
      <form ref={formRef} onSubmit={(event) => { event.preventDefault(); void saveDraft('Ficha selada como rascunho.') }}>
        ...
        <div role="tabpanel" id={`panel-${activeTab}`} aria-labelledby={`tab-${activeTab}`}>
          ...
          {activeTab === 'habilidades' && <SheetSection title="Habilidades" eyebrow="III / aquilo que foi aprendido"><div className="skill-list">{['Armas de fogo', 'Atletismo', 'Furtividade', 'Investigação', 'Liderança', 'Medicina', 'Ocultismo', 'Persuasão', 'Sobrevivência'].map((skill) => <label key={skill} className="skill-row"><span>{skill}</span>{dots(5, 0, () => {})}</label>)}</div></SheetSection>}
