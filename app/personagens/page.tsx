...
export default async function PersonagensPage() {
  ...
  const userIsMaster = isMaster(session.user)
  const campaigns = userIsMaster && process.env.DATABASE_URL ? await getCampaignsForMaster(session.user.sub) : []
  ...
  return <main className="relative min-h-screen overflow-hidden bg-background text-foreground">
    ...
    <header className="sheet-header"><div><p className="eyebrow">Arquivos da Noite</p><h1 className="mt-2 font-serif text-2xl uppercase tracking-[0.12em]">Personagens</h1></div><div className="flex items-center gap-4"><span className="hidden text-sm text-muted-foreground sm:inline">{displayName}</span>{userIsMaster && <MasterPanel initialCampaigns={campaigns} />}<a href="/auth/logout" className="button-ghost inline-flex items-center gap-2"><LogOut className="size-4" /> Sair</a></div></header>
