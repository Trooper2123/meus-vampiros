    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 lg:py-14">
      ...
      <form ref={formRef} onSubmit={(event) => { event.preventDefault(); void saveDraft('Ficha selada como rascunho.') }}>
        ...
        <div role="tabpanel" id={`panel-${activeTab}`} aria-labelledby={`tab-${activeTab}`}>
          ...
          {activeTab === 'disciplinas' && <SheetSection title="Disciplinas" eyebrow="IV / dons do sangue"><p className="mb-5 text-xs text-muted-foreground">Cada bolinha desbloqueia um poder. Escolha a disciplina, depois registre seu nome e o efeito correspondente.</p><div className="discipline-list">{disciplines.map((discipline) => <article className="discipline-card" key={discipline.id}><div className="discipline-card-header"><label className="sheet-field flex-1"><span>Disciplina</span><select className="select" name={`${discipline.id}-nome`} value={discipline.name} onChange={(event) => setDisciplines((current) => current.map((item) => item.id === discipline.id ? { ...item, name: event.target.value } : item)}><option value="">Selecione uma disciplina</option>{disciplineOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>{dots(5, discipline.level, (value) => setDisciplines((current) => current.map((item) => item.id === discipline.id ? { ...item, level: value } : item)), `${discipline.id}-nivel`)}</div>{discipline.level > 0 && <div className="power-list">{Array.from({ length: discipline.level }, (_, index) => <div className="power-row" key={index}><span className="power-level">{index + 1}</span><label className="sheet-field"><span>Poder ganho</span><input name={`${discipline.id}-poder-${index + 1}`} value={discipline.powers[index]} onChange={(event) => setDisciplines((current) => current.map((item) => item.id === discipline.id ? { ...item, powers: item.powers.map((power, powerIndex) => powerIndex === index ? event.target.value : power) } : item))} placeholder={`Poder de nível ${index + 1}`} /></label><label className="sheet-field"><span>Efeito</span><textarea name={`${discipline.id}-efeito-${index + 1}`} value={discipline.effects[index]} onChange={(event) => setDisciplines((current) => current.map((item) => item.id === discipline.id ? { ...item, effects: item.effects.map((effect, effectIndex) => effectIndex === index ? event.target.value : effect) } : item))} rows={2} placeholder="Descreva o efeito..." /></label></div>)}</div>}</article>)}</div>{disciplines.length < 7 && <button type="button" onClick={addDiscipline} className="button-ghost mt-4 inline-flex items-center gap-2"><Plus className="size-4" /> Adicionar Disciplina</button>}</SheetSection>}
